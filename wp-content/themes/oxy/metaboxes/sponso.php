...
class SponsoMetaBox {
	...
	const META_KEY = 'oxy_sponso';
	const NONCE = '_oxy_sponso_nonce';
	...
	public static function add ($postType, $post) {
		if ($postType === 'post' && current_user_can('publish_posts', $post)) {
			add_meta_box('self::META_KEY', 'Parrainage', [self::class, 'render'], 'post', 'side');
		}
	}
	...
	public static function render ($post) {
		...
		<label for="oxysponso">Cet article est parrain&eacute;/sponsoris&eacute; ?</label>
		<?php
	}
